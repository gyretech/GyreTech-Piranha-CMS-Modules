...
namespace Piranha.AspNetCore.Identity.EF.Manager
{
    public class RequiredIfAttribute : ContingentValidationAttribute
    {
        ...
        {
            if(dependentValue != null && DependentValue != null && dependentValue.GetType() != DependentValue.GetType())
            {
                dependentValue = Convert.ChangeType(dependentValue, DependentValue.GetType());
            }
